...
def get_data(network_data, feature_dic):
    ...
    temp_edge_types = list(network_data.keys())
    neighbors = generate_neighbors(network_data, vocab, num_nodes, temp_edge_types, neighbor_samples)
...
def GAMHN_model(training_data_by_type, feature_dic):
    ...
    index2label = []
    for i in index2word:
        if i in labeled_user:
            index2label.append(user_label[i])
        else:
            index2label.append(None)
    ...
    for epoch in range(epochs):
        ...
        data_iter = tqdm(
            batches,
            desc=f"epoch: {epoch}, f1: {best_f1}, acc: {best_acc}",
        ...
        for i, data in enumerate(data_iter):
            ...
            for j, ind in enumerate(data[0].tolist()):
                if index2label[ind] != None:
                    data_label.append(index2label[ind])
...
# classification valid
        acc, f1, auc, loss = classification_valid(model, classifier, valid_true_data_by_edge, feature_dic, epoch)
        if f1 > best_f1:
            best_acc = acc
            best_f1 = f1
            best_parament_dict = {
            ...
            torch.save(model.state_dict(), 'temp_data/model.pt')
            torch.save(classifier.state_dict(), 'temp_data/classifier.pt')
        with open('validation_info.txt', 'a') as file:
            file.writelines(f'acc: {acc}, f1: {f1}, loss: {loss}, auc: {auc}')
...
def classification_valid(model, classifier, valid_true_data_by_edge, feature_dic, epoch):
    ...
    split = pd.read_csv(data_path + 'split.csv', index_col='id')
    test_user = split.groupby('split').groups['test'].tolist()
    valid_index2label = []
    for i in valid_index2word:
        if i in test_user:
            valid_index2label.append(user_label[i])
        else:
            valid_index2label.append(None)
    ...
    for i, valid_data in enumerate(valid_data_iter):
        ...
        temp_test_user_index = []
        data_label = []  # 数据的标签
        for j, ind in enumerate(valid_data[0].tolist()):
            if valid_index2label[ind] != None:
                data_label.append(valid_index2label[ind])
                temp_test_user_index.append(j)
        if len(data_label) == 0:  # 本batch中无标注的用户
            continue

        data_label = torch.tensor(data_label, dtype=torch.float).to(device)
        valid_user_embs = valid_embs[temp_test_user_index]

        predict = classifier(valid_user_embs)
        class_loss = classifier_loss(predict, data_label)

        all_label += data_label.tolist()
        all_predict += predict.tolist()
    ...
    all_predict = [pred >= 0.5 for pred in all_predict]
    return accuracy_score(all_label, all_predict), f1_score(all_label, all_predict), \
           roc_auc_score(all_label, all_predict), class_avg_loss


if __name__ == "__main__":
    args = parse_args()
    file_name = args.input      # NOTE change the file path in utils.py
    data_path = r"/data1/botdet/datasets/Twibot-20-Format/"
    print(args)
    ...
    with open(file_name + 'val_edge.json', 'r') as fp:
        valid_true_data_by_edge = json.load(fp)

    print("\ninitialize model")
    edge_types = list(training_data_by_type.keys())
    user_feature_dim = torch.tensor(15, dtype=torch.long)       # FIXME
    text_feature_dim = torch.tensor(32, dtype=torch.long)

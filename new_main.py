...
def get_batches(pairs, neighbors, batch_size, index2word, sub_feature_dic):
    ...
    for idx in range(n_batches):            # 第idx个batch
        ...
        yield torch.tensor(x), torch.tensor(y), torch.tensor(t), torch.tensor(neigh), \
              indices, torch.tensor(user_features, dtype=torch.float), torch.tensor(text_features, dtype=torch.float)
...
class GATNEModel(nn.Module):
    ...
    def forward(self, train_inputs, train_types, node_neigh, indices, user_features, text_features, neighbors_features):
        ...
        # node_embed = torch.stack([user_node_embed, text_node_embed])  # FIXME 之前写错了
...
def get_data(network_data, feature_dic):
    ...
    for node in index2word:
        ...
        if len(sub_feature_dic[node]) != 64:
            print("no")
...
def GAMHN_model(training_data_by_type, feature_dic):
    ...
    try:
        import pickle
        with open('temp_data/one_batch.pickle', 'wb') as f:
            pickle.dump([index2word, train_pairs, neighbors, num_nodes, sub_feature_dic, neighbors_features], f)
        print('successfully pickle')
...
def classification_valid(model, classifier, valid_true_data_by_edge, feature_dic, epoch):
    model.eval()
    classifier.eval()
    valid_index2word, valid_pairs, valid_neighbors, valid_num_nodes, valid_features, valid_neighbors_features \

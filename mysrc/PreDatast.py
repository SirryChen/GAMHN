    ...
    with open(path + 'fixed_node.json') as file:
        ...
        for text in tqdm(texts, desc='loading text properties'):
            ...
            if not text['id'].find('t') == -1:      # 推文对象
                ...
                continue
    convert_length = 100
    input_tensor = tokenizer(tweet_library, max_length=32, padding='max_length',
                             return_tensors="pt", truncation=True)  # 编码推文
    for i in range(len(tweet_library)/convert_length):
        input_tensor = tokenizer(tweet_library, max_length=32, padding='max_length',
                                 return_tensors="pt", truncation=True)  # 编码推文
    final_tweet = model(input_tensor['input_ids']).pooler_output         # 获取所有推文表示
    feature_dim = final_tweet.shape[1]
    texts_feature_dict = dict(zip(tweet_id_library, final_tweet.tolist()))


    return texts_feature_dict, feature_dim

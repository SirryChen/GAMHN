...
def get_batches(pairs, neighbors, batch_size, index2word, sub_feature_dic):
    ...
    for idx in range(n_batches):            # 第idx个batch
        ...
        for i in range(batch_size):
            ...
            if ind >= len(pairs):
                break
            if not index2word[ind].find('u') == -1:     # 用户
                indices.append(i)
                user_features.append(sub_feature_dic[index2word[ind]])
            x.append(pairs[ind][0])       # [index_i,...]
            y.append(pairs[ind][1])       # [index_j,...]
            t.append(pairs[ind][2])       # [layer_id,...] 其实是edge_type_i, index_i与index_j节点间边的关系集合
        ...
        for i in range(batch_size):
            ...
            if ind >= len(pairs):
                break
            if index2word[ind].find('u') == -1:     # 推文
                indices.append(i)
                text_features.append(sub_feature_dic[index2word[ind]])

        # 返回一个batch_size
        yield torch.tensor(x), torch.tensor(y), torch.tensor(t), torch.tensor(neigh), \
              indices, torch.tensor(user_features, dtype=torch.long), torch.tensor(text_features, dtype=torch.long)
...
class GATNEModel(nn.Module):
    def __init__(
        ...
        self.text_embed_trans = Parameter(torch.FloatTensor(user_feature_dim, embedding_size))
        self.user_embed_trans = Parameter(torch.FloatTensor(text_feature_dim, embedding_size))
    ...
    def forward(self, train_inputs, train_types, node_neigh, indices, user_features, text_features, neighbors_features):
        ...
        node_embed = torch.stack([user_node_embed, text_node_embed])
        node_embed = torch.index_select(node_embed, dim=0, index=indices)
        ...
        for i, edge_type in enumerate(self.edge_types):
            if edge_type == 'tweet':
                ...
                node_embed_neighbors.append(tweet_neighbor)
            else:
                ...
                node_embed_neighbors.append(user_neighbor)
        node_embed_neighbors = torch.stack(node_embed_neighbors)
...
def get_data(network_data, feature_dic):
    ...
    neighbors_features = torch.tensor(neighbors_features, dtype=torch.long).to(device)
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(data_iter):
            ...
            embs = model(data[0].to(device), data[2].to(device), data[3].to(device),
                         data[4], data[5].to(device), data[6].to(device), neighbors_features)
            ...
            # 获取本batch用户标签
            data_label = [user_label[index2word[ind]] for ind in data[0] if not index2word[ind].find('u') == -1]
...
def classification_valid(model, classifier, valid_true_data_by_edge, feature_dic, epoch):
    ...
    valid_index2word, valid_pairs, valid_neighbors, valid_features, valid_num_nodes \
        = get_data(valid_true_data_by_edge, feature_dic)
    valid_features = torch.FloatTensor(valid_features).to(device)

    random.shuffle(valid_pairs)
    valid_batches = get_batches(valid_pairs, valid_neighbors, batch_size)  # 源节点列表，目标节点列表，关系列表，每个源节点的所有邻居列表
    ...
    for i, valid_data in enumerate(valid_data_iter):
        valid_embs = model(valid_data[0].to(device), valid_data[2].to(device), valid_data[3].to(device),
                           valid_features)  # 源节点、关系、源节点所有邻居(neighbors中信息)
...
if __name__ == "__main__":
    ...
    training_data_by_type = json.load(file_name + 'edge.json')
    feature_dic = json.load(file_name + 'node_attribute.json')      # 所有node的特征属性，不等长
    user_label = json.load(file_name + 'user_label.json')

    edge_types = list(training_data_by_type.keys())
    user_feature_dim = 15       # FIXME
    ...
    final_acc = classification_valid(model, classifier, testing_true_data_by_edge, feature_dic, epoch=520)

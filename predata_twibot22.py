...
# FIXME change path
dataset = "Twibot-22-datasets"
path = r"/data1/botdet/datasets/" + dataset + "/"
...
def edge_build():
    ...
    with open('pre_data/{}/edge.json'.format(dataset), 'w') as f:
...
def test_edge_build(data_type='test'):
    ...
    with open('pre_data/{}/'.format(dataset) + data_type + '_edge.json', 'w') as fp:
...
def texts_feature():
    ...
    tweet_library = []
    for i in range(10):
        with open(path + 'tweet_{}.json'.format(i)) as file:
...
def get_user_label():
    ...
    with open('pre_data/{}/user_label.json'.format(dataset), 'w') as f:

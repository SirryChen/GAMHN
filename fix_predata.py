...
# FIXME change path
path = r"/data1/botdet/datasets/Twibot-20-Format/"
...
def edge_build():
    ...
    train_split_id = split.groupby('split').groups['train'].tolist()    # 存储用于训练的用户id

    chunk_edges = pd.read_csv(path + 'edge.csv', chunksize=10000)   # chunksize分批处理
    edge_type_id_id = dict()
    for edges in tqdm(chunk_edges, desc='train edge building'):
    ...
    with open('pre_data/edge.json', 'w') as f:
...
def test_edge_build(data_type='test'):
    assert data_type in ['test', 'val'], 'data type error'
    split = pd.read_csv(path + 'split.csv', index_col='id')
    ...
    for edges in tqdm(chunk_edges, desc=data_type + "edge building"):
        for _, edge in edges.iterrows():
    ...
    with open('pre_data/' + data_type + '_edge.json', 'w') as fp:
...
def texts_feature():
    ...
    #     with open(r"/data1/botdet/datasets/Twibot-20/USE_predata/USE_temp_data{}.json".format(i), 'w') as f:
    #         # NOTE BERT
    ...
    for i in tqdm(range(convert_step)):
        with open(r"/data1/botdet/datasets/Twibot-20/USE_predata/USE_temp_data{}.json".format(i), 'r') as f:
            temp_texts_feature_dict = json.load(f)
            for key in temp_texts_feature_dict.keys():
                temp_texts_feature_dict[key] = temp_texts_feature_dict[key][0:text_feature_dim]
                count += 1
            final_texts_feature_dict = dict(**final_texts_feature_dict, **temp_texts_feature_dict)

    print('tweet count:{}'.format(count))
    return final_texts_feature_dict
...
def get_user_label():
    ...
    with open('pre_data/user_label.json', 'w') as f:
...
if __name__ == '__main__':
    ...
    # valid_edge_type_id_id = test_edge_build("val")
    # test_edge_type_id_id = test_edge_build("test")

    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

    texts_feature_dict = texts_feature()
    users_feature_dict = users_feature(properties_list, properties_number, text_feature_dim)
    feature_dict = dict(**users_feature_dict, **texts_feature_dict)     # 合并字典
    with open('pre_data/node_attribute.json', 'w') as fp:
